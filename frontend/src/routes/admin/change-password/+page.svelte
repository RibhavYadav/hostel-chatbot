<script lang="ts">
	...
	import { adminChangePassword } from '$lib/services/api';
	...
	/**
	 * Handles admin password change form submission.
	 * Validates all fields before calling POST /admin/change-password.
	...
	async function handleSubmit(): Promise<void> {
		...
		isLoading = true;
		try {
			const result = await adminChangePassword(currentPassword, newPassword, confirmNewPassword);
			successMessage = result.message;
			setTimeout(() => goto(resolve('/admin/dashboard')), 2000);
		} catch (error) {
	...
	<div class="auth-card">
		<div class="text-center">
			...
			<p class="mt-1 text-sm text-slate-500">
				{$adminAuthStore.currentAdmin?.emailID}
			</p>
		</div>
